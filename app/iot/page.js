    ...
    useEffect(() => {
        ...
        if (isSimulating) {
            interval = setInterval(() => {
                const randomSpeed = Math.max(0, Math.min(120, formData.speed + (Math.random() * 20 - 10)));
                const randomTemp = formData.temperature + (Math.random() * 4 - 2);
                const randomHum = Math.max(0, Math.min(100, formData.humidity + (Math.random() * 10 - 5)));
                ...
                <div className="card">
                    <div className="flex items-center justify-between mb-8 border-b border-[#334155] pb-4">
                        <h2 className="text-xl font-semibold flex items-center text-[#F8FAFC]">
                            <Cpu className="mr-3 text-[#4F46E5]" />
                            Device Control
                        </h2>
                        <div className={`flex items-center space-x-2 px-3 py-1 rounded-full text-xs font-medium border ${isSimulating ? 'bg-[#22C55E]/10 text-[#22C55E] border-[#22C55E]/20' : 'bg-[#1E293B] text-[#94A3B8] border-[#334155]'}`}>
                            <div className={`w-2 h-2 rounded-full ${isSimulating ? 'bg-[#22C55E] animate-pulse' : 'bg-[#94A3B8]'}`}></div>
                ...
                <div className="card flex flex-col h-[600px]">
                    <div className="flex items-center justify-between mb-4 border-b border-[#334155] pb-2">
                        <h2 className="text-lg font-semibold flex items-center text-[#F8FAFC]">
                            <Terminal className="mr-2 w-4 h-4 text-[#94A3B8]" />
                            System Logs
                        </h2>
                    ...
                    <div className="flex-1 overflow-y-auto space-y-2 pr-2 custom-scrollbar font-mono text-sm">
                        {logs.length === 0 && (
                            <div className="text-[#94A3B8] mt-20 text-center">
